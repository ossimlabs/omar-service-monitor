...
class ServiceMonitor extends Component {
  ...
  fetchServices = () => {
    setInterval(() => {
      ...
      fetch(
        `${this.props.server}/omar-eureka-server/eureka/vips/${appName}`,
        {
          headers: {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <p className="service-name">{this.props.app.name} {this.state.version}</p>
          ...
          <img className="right" src="https://jenkins.ossim.io/buildStatus/icon?job=omar-oms-dev" alt=""/>
      </React.Fragment>
    );
  }
}
